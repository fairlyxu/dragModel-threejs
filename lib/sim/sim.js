...
Sim.App.prototype.objectFromMouse = function(pagex, pagey) {
    ...
    console.info(this.objects.length + " aaaaaaaaaaa " + this.objects[0] instanceof(THREE.Mesh));

    // intersection = (intersects.length) > 0 ? intersects[0] : null;
    console.info(intersects + " aaaaaaaaaaa " + intersects.length);



    var intersected = (intersects.length) > 0 ? intersects[0] : null;

    console.info(intersected + " ssssssssssss ");
    if (null !== intersected) {
...
Sim.Object.prototype = new Sim.Publisher; //类Object 可以调用类Publisher的方法和属性
